...
export const JourneySection = () => {
  ...
  return (
    <section id="journey" className={`py-20 ${timeBasedClass}`}>
      ...
      >
        <h2 className={`
          text-3xl sm:text-4xl font-bold text-center mb-10 transition-colors duration-300 ${!isLightMode ? 'synthwave-text-glow' : ''} ${textClass}                         
        `}>
          My Journey
        </h2>
        ...
        <Tabs defaultValue="education" className="w-full">
          <TabsList className="grid grid-cols-3 w-[85vw] max-w-[800px] lg:w-full mb-8 bg-transparent gap-1 sm:gap-2 p-1 border rounded-full backdrop-blur-sm shadow-inner overflow-hidden mx-auto h-auto" style={{ borderColor: isLightMode ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.1)', background: isLightMode ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)' }}>
            <TabsTrigger
              value="education"
              className={`rounded-full flex-1 w-full flex items-center justify-center text-[10.5px] xs:text-[11px] sm:text-sm whitespace-nowrap px-0 sm:px-3 py-2 md:py-2.5 h-full data-[state=active]:bg-[var(--theme-color)] data-[state=active]:text-white transition-all duration-300 ${textClass}`}
            >
              Education
            </TabsTrigger>
            <TabsTrigger
              ...
              <div className="flex flex-col">
                {education.map((edu, index) => (
                  <div key={edu.id} className={`relative pb-8 mb-8 border-b ${isLightMode ? 'border-black/10' : 'border-white/10'} last:border-0 last:pb-0 last:mb-0`} style={{ animationDelay: `${index * 0.1}s` }}>
                    <div className="mb-4">
                      <div>
                        <h3 className={`text-lg sm:text-xl font-bold transition-colors duration-300 ${textClass}`}>
                          {edu.degree}
                        </h3>
                        <p className={`text-base sm:text-lg font-semibold transition-colors duration-300 ${textClass}`}>
                          {edu.institution}
                        </p>
                        <p className={`text-xs sm:text-sm transition-colors duration-300 ${textClass} opacity-70`}>
              ...
              <div className="flex flex-col">
                {extracurriculars.map((activity, index) => (
                  <div key={activity.id} className={`relative pb-8 mb-8 border-b ${isLightMode ? 'border-black/10' : 'border-white/10'} last:border-0 last:pb-0 last:mb-0`} style={{ animationDelay: `${index * 0.1}s` }}>
                    <div className="mb-4">
                      <h3 className={`text-lg sm:text-xl font-bold mb-2 transition-colors duration-300 ${textClass}`}>
                        {activity.position}
                      </h3>
                      <p className={`text-base sm:text-lg font-semibold transition-colors duration-300 ${textClass}`}>
