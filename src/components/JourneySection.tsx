    ...
    <section id="journey" className={`py-20 px-6 ${timeBasedClass}`}>
      ...
      >
        ...
        <Tabs defaultValue="education" className="w-full">
          <TabsList className="grid grid-cols-3 w-full mb-8 bg-transparent gap-1 sm:gap-2 p-1 border rounded-full backdrop-blur-sm shadow-inner overflow-hidden mx-auto max-w-3xl h-auto" style={{ borderColor: isLightMode ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.1)', background: isLightMode ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)' }}>
            <TabsTrigger
              value="education"
              className={`rounded-full flex-1 w-full text-[11px] sm:text-sm whitespace-normal sm:whitespace-nowrap px-1 sm:px-3 py-2 data-[state=active]:bg-[var(--theme-color)] data-[state=active]:text-white transition-all duration-300 ${textClass}`}
            >
              Education
            </TabsTrigger>
            <TabsTrigger
